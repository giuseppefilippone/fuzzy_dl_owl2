...
extensions = [
    ...
    "sphinx_uml_autodoc",  # ← custom extension to generate UML class diagrams from code
...
exclude_patterns = ["_uml", "md", "old_md"]
...
# autodoc_typehints = "both"  # show type hints in signature and description
...
latex_documents = [
    (
        "index",  # startdocname
        "fuzzy_dl_owl2-documentation.tex",  # targetname  ← avoids "fuzzydlowl2.tex/pdf"
        "Fuzzy Description Logic OWL 2 Documentation",  # title
        "Giuseppe Filippone",  # author
        "manual",  # theme
    ),
]

# Ship latexmkrc into the LaTeX build dir to cap pdflatex passes.
# autoapi generates cross-references that never resolve, causing
# latexmk to loop indefinitely without this.
latex_additional_files = ["latexmkrc"]
