...
logger.debug(
    "[uml] Sphinx UML Autodoc extension loaded (name=%s, version=%s)",
    __name__,
    __version__,
)
...
def _on_builder_inited(app: Sphinx) -> None:
    logger.info("[uml] Starting UML diagram generation...")
    ...
    for fqn, src_file in classes:
        ...
        logger.debug("[uml] Processing %s...", safe)
...
def _pyreverse_class(
    ...
    logger.debug("[uml] Attempting to generate diagram for %s using pyreverse ...", fqn)
    logger.debug(
    ...
    with tempfile.TemporaryDirectory(prefix="uml_") as tmpdir:
        ...
        try:
            logger.debug(
                "[uml] PYTHONPATH for pyreverse: %s", env.get("PYTHONPATH", "")
            )
            logger.debug("[uml] Running command: %s", " ".join(cmd))
            r = subprocess.run(
                cmd,
                capture_output=True,
                text=True,
