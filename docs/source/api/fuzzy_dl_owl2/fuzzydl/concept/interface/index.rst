...
* ``fuzzy_dl_owl2.fuzzydl.concept.interface.has_concept_interface`` — An abstract base class providing a standard interface for objects to manage and update a mutable conceptual entity.
* ``fuzzy_dl_owl2.fuzzydl.concept.interface.has_concepts_interface`` — An abstract base class that provides a standardized mechanism for storing, retrieving, and updating a mutable collection of Concept objects.
* ``fuzzy_dl_owl2.fuzzydl.concept.interface.has_role_concept_interface`` — An abstract base class that establishes a contract for objects requiring both a functional role and an associated concept.
* ``fuzzy_dl_owl2.fuzzydl.concept.interface.has_role_interface`` — An abstract base class defines a standard interface for managing a role attribute within an object.
* ``fuzzy_dl_owl2.fuzzydl.concept.interface.has_value_interface`` — An abstract base class extends role management capabilities by introducing a generic value attribute with deep copy protection.
* ``fuzzy_dl_owl2.fuzzydl.concept.interface.has_weighted_concepts_interface`` — An abstract base class that defines a contract for managing a collection of concepts alongside associated numerical weights.
