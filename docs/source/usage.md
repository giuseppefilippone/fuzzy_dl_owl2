...
The optional `maxVal` setting controls the Big-M constant used in the datatype-restriction rows of the MILP program. By default (omitted, empty, or set to `auto`), the reasoner derives a suitable Big-M automatically from the numeric magnitudes the knowledge base constrains — the declared feature ranges plus the thresholds and fuzzy breakpoints the parser collects while reading the axioms. Setting an explicit value (for example `maxVal = 1000000`) forces that Big-M, which is only needed when the knowledge base legitimately declares thresholds or data values of a magnitude comparable to the automatic one.
