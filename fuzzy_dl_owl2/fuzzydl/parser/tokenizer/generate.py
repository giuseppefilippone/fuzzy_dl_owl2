...
def gen_lexer_flex() -> str:
    ...
    lines = [
        ...
        'static size_t g_n, g_off;',
        '#define EMIT(t) do { g_types[g_n]=(t); g_starts[g_n]=(int32_t)g_off; g_lens[g_n]=(int32_t)yyleng; g_n++; g_off+=yyleng; } while (0)',
        "%}",
        "",
        "%option noyywrap",
        "",
        "%%",
        "",
        '[ \\t\\r\\n\\f\\v"]+                 /* skip whitespace and quotes */',
        "(#|%).*                              /* skip comments */",
    ]
    for name in kw:
        lines.append(
