...
class FeatureFunction:
    ...
    def __init__(self, *args) -> None:
        """
        Initializes a feature function object by dispatching to specific internal constructors based on the type and count of the provided arguments. The method accepts either a single argument or a pair of arguments. When given a single argument, it may be a string (interpreted as a feature name), a numeric constant, an existing FeatureFunction instance, or a sequence of FeatureFunction instances. When provided with two arguments, the method expects either two FeatureFunction instances or a numeric constant followed by a FeatureFunction instance. If the arguments do not match these specific type signatures or if the argument count is incorrect, a ValueError is raised.

        :param args: Variable length argument list supporting multiple initialization patterns: a single FeatureFunction, string, number, or sequence of FeatureFunctions; or a pair of two FeatureFunctions or a number and a FeatureFunction.
        :type args: typing.Any

        :raises ValueError: Raised when the provided arguments do not match any of the supported initialization signatures. Valid inputs include a single FeatureFunction, string, number, or sequence of FeatureFunctions, or a pair of FeatureFunctions, or a number and a FeatureFunction.
        """
