...
class Inequation:
    ...
    def __init__(self, exp: Expression, i_type: InequalityType) -> None:
        """
        Constructs an `Inequation` object defined by a specific mathematical expression and a relational operator type. The method validates that the provided expression is not `None` and contains at least one term, ensuring the inequality is well-formed. Upon successful validation, the expression and inequality type are stored as instance attributes for subsequent use in algebraic manipulations or comparisons.

        :param exp: The mathematical expression defining the inequality.
        :type exp: Expression
        :param i_type: Specifies the type of inequality relation (e.g., less than, greater than).
        :type i_type: InequalityType
        """
    ...
    def is_zero(self) -> bool:
        ...
        return (
            all(term.get_coeff() == 0 for term in self.expr.get_terms())
            and self.expr.get_constant() == 0
        )
